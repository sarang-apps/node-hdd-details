...
function extractDiskNum(details) {
    ...
    if (diskNumberArray.length == 1) {
        details.partition.disk = diskNumberArray[0]
        return diskNumberArray[0]
    } else {
        ...
        .then(async disks => {
            ...
            if(filtered_disks.length == 1) {
                ...
                if(diskNumberArray && diskNumberArray.length == 1) {
                    details.partition.disk = diskNumberArray[0]
                    filtered_disks = disks.filter(disk => disk.device == `disk${diskNumberArray[0]}` )
                    if(filtered_disks.length == 1) {
                        details.disk = filtered_disks[0]
                    }
                }
...
// diskInfo("/Volumes/Backup/Shastri Backup")
